      ...
      <ul class="purchaseUl">
        <li @click="togglePurchase()">
          <div  :class="purchaseShow ==false ? 'perTopDiv':'perDiv'">
            <p class="contentP">{{interfaceContent.txt}}</p>
            <em class='icon3'></em>
            <p class="disInlieBlock">{{interfaceContent.date | formatDateNew}}</p>
          </div>
        </li>
        <!--明细区域开始-->
        <div  v-if="!purchaseShow" class="detailArea">
          <div>
            <table class="detailList" id='detail'>
              <tr><td>标题：</td><td>{{interfaceContent.txt}}</td></tr>
              <tr><td>创建者：</td><td>{{interfaceContent.name}}</td></tr>
              <tr><td>创建时间：</td><td>{{interfaceContent.date | formatDateNew}}</td></tr>
...
/* eslint-disable */
  ...
  export default {
    ...
    created: function () {
      ...
      this.interfaceContent = this.$route.query
    },
    filters: {
      formatDateNew (time) {
